; "Virtual Reality / Critical IF ZIL Gamebook Engine v2.1"
; ---------------------------------------------------------------------------------------------
...
<ROUTINE GET-FROM-LIST (ITEM "OPT" CONTAINER "AUX" ITEMS COUNT)
...
<ROUTINE LOSE-STUFF (CONTAINER LOST-CONTAINER ITEM "OPT" MAX ACTION "AUX" (COUNT 0) ITEMS)
	...
	<COND (<G? <COUNT-CONTAINER .CONTAINER> .MAX>
		...
		<DO (I 1 .ITEMS)
			<SET COUNT <+ .COUNT 1>>
			<COND (<L=? .COUNT .ITEMS>
				<PUT TEMP-LIST .COUNT <GET-FROM-LIST .I .CONTAINER>>
			)>
		>
		<REPEAT ()
